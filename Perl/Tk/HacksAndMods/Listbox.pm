...
#JWT:OVERRIDDEN FUNCTIONS TO MAKE LISTBOXES BEHAVE MORE NATURALLY (TO ME):
#JWT:(YOU MUST DO:  "export jwtlistboxhack=1" IN YOUR ENVIRONMENT TO USE, 
#     OTHERWISE LISTBOX BEHAVES AS NORMAL)
...
sub jwtUpDown
{
	...
	if ($ENV{'jwtlistboxhack'}) {
		...
		#SIGNAL EVENT (for Tk::JBrowseEntry, et. al.) THAT WE'RE ALREADY AT TOP OR BOTTOM OF LIST -
		#THIS WAY, WE CAN DO COOL STUFF LIKE UNPOP THE DROPDOWN LIST, CREATE A "CIRCULAR LIST", ETC.
		if ($amount < 0 && defined($active) && $active <= 0) {
			$w->eventGenerate('<<ListboxTop>>');
			return;
		} elsif ($active+$amount >= $w->index('end')) {
			$w->eventGenerate('<<ListboxBottom>>');
			return;
		}
		$w->activate($w->index('active')+$amount);
		$w->see('active');
		if ($w->cget('-selectmode') =~ /^browse/o)
		{
...
sub jwtCtrlUpDown   #DEPRECIATED:  JWT:  ADDED 20091020 FOR EXTENDED SELECTION VIA KEYBOARD:ADDED 20091020 NEXT 2 FOR EXTENDED SELECTION VIA KEYBOARD:
{
 my $w = shift;
 my $amount = shift;
 $w->activate($w->index('active')+$amount);
...
sub jwtBeginSelect   #Mouse button-press (button 1)
{
	my $w = shift;
	if ($ENV{'jwtlistboxhack'}) {
		my $el = shift;
...
sub jwtExtendUpDown
{
	...
	if ($ENV{'jwtlistboxhack'}) {
		...
		return $w->UpDown($amount)  if ($w->cget('-selectmode') !~ /^extended/o);
...
sub jwtSpaceSelect    #JWT:ADDED TO ALLOW <SPACEBAR> TO SELECT INDIVIDUAL ITEMS IN "EXTENDED" SELECTIONS:
{
	my $w = shift;
...
sub jwtShiftSpace  #Shift-spacebar pressed:  Select from anchor to active inclusive:
{
	my $w = shift;
	my $el = shift;
	...
	if (defined($anchor) && $mode =~ /^extended/o
		...
		if ($w->selectionIncludes($el)) {  #TOGGLE SELECT-STATUS OF ENTRY CLICKED ON:
			...
			$w->selectionClear(0, 'end')  if ($mode =~ /^(?:single|browse)$/o);
			$w->selectionSet($el);
		}
	}
	$w->selectionAnchor($el);
...
sub ClassInit
{
 ...
 $mw->bind($class,'<Alt-ButtonRelease-1>','jwtAltButtonRelease_1');
 $mw->bind($class,'<Shift-1>',['jwtBeginExtend',Ev('index',Ev('@'))]);
 $mw->bind($class,'<Control-1>',['BeginToggle',Ev('index',Ev('@'))]);

 $mw->bind($class,'<B1-Leave>',['AutoScan',Ev('x'),Ev('y')]);
 $mw->bind($class,'<B1-Enter>','CancelRepeat');
 $mw->bind($class,'<Up>',['jwtUpDown',-1]);
 $mw->bind($class,'<Shift-Up>',['jwtExtendUpDown',-1]);
 $mw->bind($class,'<Down>',['jwtUpDown',1]);
 $mw->bind($class,'<Shift-Down>',['jwtExtendUpDown',1]);
...
# $mw->bind($class,'<space>',['BeginSelect',Ev('index','active')]);  #JWT:CHGD TO NEXT 20091020 FOR EXTENDED SELECTION VIA KEYBOARD:
 $mw->bind($class,'<space>',['jwtSpaceSelect',Ev('index','active')]);
 $mw->bind($class,'<Select>',['jwtBeginSelect',Ev('index','active')]);
 $mw->bind($class,'<Control-Shift-space>',['jwtBeginExtend',Ev('index','active')]);
 if ($ENV{'jwtlistboxhack'}) {
	$mw->bind($class,'<Shift-space>',['jwtShiftSpace',Ev('index','active')]);  #JWT:ADDED 20091020!
	$mw->bind($class,'<Alt-space>',['jwtAltSpace',Ev('index','active')]);  #JWT:ADDED 20091020!
#DEPRECIATED: 	$mw->bind($class,'<Control-Up>', sub { my $self = shift; $self->jwtCtrlUpDown(-1) });   #JWT:ADDED 20091020 NEXT 2 FOR EXTENDED SELECTION VIA KEYBOARD:
#DEPRECIATED: 	$mw->bind($class,'<Control-Down>', sub { my $self = shift; $self->jwtCtrlUpDown(1) });
 }
 $mw->bind($class,'<Shift-Select>',['jwtBeginExtend',Ev('index','active')]);
 $mw->bind($class,'<Escape>','jwtCancel');
